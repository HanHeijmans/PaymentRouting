...
public class SplitCloseTiesLookahead extends PathSelection {

    public SplitCloseTiesLookahead(DistanceFunction df) {
        super("SPLIT_CLOSE_TIES_LOOKAHEAD", df);
    }

    public SplitCloseTiesLookahead(String key, DistanceFunction df) {
        super(key, df);
    }
            ...
            for (int i = 0; i < vals.length; i++) {
                ...
                while (vec.size() > 0) {
                    ...
                    for(int j = 0; j < vec.size(); j++) {
                        if(this.computeTotalPotential(g, out[vec.get(j)], dst, cur, excluded, rp, curVal, rand, reality) > this.computeTotalPotential(g, out[node], dst, cur, excluded, rp, curVal, rand, reality)) {
                            node = vec.get(j);
                            index = j;
                        }
                    }
